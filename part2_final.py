# -*- coding: utf-8 -*-
"""
Created on Sun Aug 16 00:07:12 2020
...
class MyMind:
    ...
    def decide(self):
        ...
        if self.should_clean:
            return action.speak([1, self.parsed_observations]), action.clean()

        if self.grid_size_broadcast == 1:
            return action.speak([2, self.parsed_observations, self.grid_size]), action.clean() if self.should_clean else action.turn(direction.right)


        if self.grid_size == 0:
            return action.speak([1, self.parsed_observations]), self.find_grid_size()
     
        if self.grid_size_broadcast == 1:
            return action.speak([2, self.parsed_observations, self.grid_size]), action.clean() \
                if self.should_clean else action.turn(direction.right)


        if not self.on_dest():
            return action.speak([1, self.parsed_observations]), self.move_to_destination() 
    ...
    def move_towards_dest_yx(self, dest):
        if self.current_position.y < dest[1]:
            if self.orientation != orientation.south:
                if self.orientation == orientation.north:
                    return action.turn(direction.right)
                if self.orientation == orientation.west:
        ...
        if self.current_position.x < dest[0]:
            if self.orientation != orientation.east:
                if self.orientation == orientation.north:
                    return action.turn(direction.right)
                if self.orientation == orientation.south:
    ...
    def move_towards_dest_xy(self, dest):
    ...
    def revise(self, observation, messages):
        ...
        self.current_position = observation.center.coordinate
        ...
        if observation.center.dirt: #check for dirt
            if observation.center.dirt.colour is self.colour:
                self.should_clean = 1
       
        self.parsed_observations = self.parse_observations(observation)
        self.update_grid(self.parsed_observations)
        ...
        if self.grid_size == 0:
            ...
            if self.current_position.x > self.current_position.y:
    ...
    def check_messages(self,observation,messages):
        for msg in messages:
            ...
            if msg.content[0] == 0:
                self.update_grid(msg.content[1], msg.sender)
    ...
    def move_to_destination(self):
        ...
        if self.orientation_destination_order == "yx":
            return self.move_towards_dest_yx(dest)
        
        return self.move_towards_dest_xy(dest)
    def control_movement_logic(self,observation):
        ...
        if self.destination is None or self.on_dest():
            ...
            if closest_unexplored and closest_colour is None:
                ...
                if self.find_eucledian_distance((self.current_position.x, self.current_position.y), closest_colour) \
                        < self.find_eucledian_distance((self.current_position.x, self.current_position.y), closest_unexplored):
                    self.destination = closest_colour
                    return
    ...
    def on_dest(self):
        """
        This is used to check if the agent has reached the set traversal/unblock destination
      
        """
        return self.find_eucledian_distance((self.current_position.x, self.current_position.y), self.give_way) == 0.0 \
            if self.give_way \
            else self.find_eucledian_distance((self.current_position.x, self.current_position.y), self.destination) == 0.0

    @staticmethod
    def find_eucledian_distance(x, y):
        """
        Calculates the closest Euclidean distance between two points
    ...
    def get_closest(self, obj_type=None):
        ...
        if len(locations) == 0:
            return
        locations.sort(key=lambda x: self.find_eucledian_distance((self.current_position.x, self.current_position.y), x))
    ...
    def update_grid(self, obs, ag=None):
    ...
    def check_forward_movement(self,obs):
        if obs.forward == None and (obs.left and obs.right):
            ...
            return True
        return False
    def opposite_orientation(self,o):
        if o is orientation.east:
            return orientation.west
        if o is orientation.west:
            return orientation.east
        if o is orientation.north:
            return orientation.south
        if o is orientation.south:
